	...
	return !isLoading ? (
      ...
      <Styled.Content>
        <Styled.FormContainer>
          {
            iscorrectAnswer ? <Styled.SuccessMessage>Bien joué ! 😁🚀</Styled.SuccessMessage> : (
              <Fragment>
                ...
                <Styled.Form onSubmit={(e) => onAnswerSubmit(e)}>
                  ...
                  {solutionToShow ?
                  <div>
                    <Styled.ErrorMessage>Malheureusement c'est la mauvaise réponse 😥</Styled.ErrorMessage>
                    {
                      !toggleSolution
                        ? <Styled.ToggleSolution onClick={onToggleSolution}>Du mal à trouver la réponse ? Tu peux cliquer ici 🙋</Styled.ToggleSolution>
                        : <div>{solution}</div>
                    }
                  </div>
                    : null}
        ...
        <Styled.TipsContainer>
          {currentQuestion.length !== questionLength && <Styled.Timer>{`⌛ | Prochain indice dans: ${10 - secondsToString.charAt(1)}`}</Styled.Timer>}
          {currentQuestion.map(({ step, indice }, index) => {
            return <Styled.Tips key={index}>{`${step}: ${indice}`}</Styled.Tips>
          })}
          {currentQuestion.length === questionLength && <Styled.CatchPhrase>Je suis, je suis, je suiiiiiiiiiis ....</Styled.CatchPhrase>}
        </Styled.TipsContainer>
      </Styled.Content>
		</Styled.Root>
	) : (
